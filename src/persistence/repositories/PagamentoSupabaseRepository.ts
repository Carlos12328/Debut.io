...
export class PagamentoSupabaseRepository implements PagamentoRepository {
  async getByFornecedor(id_fornecedor: EntityId): Promise<Pagamento[]> {
    const { data } = await supabase.from('pagamento').select('*').eq('id_fornecedor', id_fornecedor);
    return data ?? [];
  }
  ...
  async create(data: Pagamento): Promise<Pagamento> {
    const { data: novo } = await supabase
      .from('pagamento')
      .insert({ id_fornecedor: data.id_fornecedor, valor: data.valor, vencimento: data.vencimento, status: data.status })
      .select().single();
    return novo!;
  }
